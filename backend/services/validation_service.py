...
# Non-standard labels accepted from upstream — mapped to canonical values
# before preflight cross-field validation runs.
...
def run_preflight(
    ...
    # Rule: severity must be from allowed values (normalise non-standard labels first)
    if "severity" in df.columns:
        df["severity"] = df["severity"].map(
            lambda v: SEVERITY_NORMALISE.get(str(v), str(v)) if v is not None else v
        )
    invalid_severity = df["severity"].dropna()
    invalid_severity = invalid_severity[~invalid_severity.isin(VALID_SEVERITIES)]
    if len(invalid_severity) > 0:
