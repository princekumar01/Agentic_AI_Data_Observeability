...
import { segDashboardApi as dashboardApi } from '../lib/api';
import AnalyticalDashboard from './Dashboard';
import {
  ApprovedRun, RunKpis, NullRateColumn, SeverityItem,
  AgentConfidence, AnomalySummaryItem, SegregatedTokenUsage, Pillar, Finding,
} from '../lib/types';
...
function SegregatedDashboard({ runId }: { runId: string }) {
  ...
  useEffect(() => {
    ...
    dashboardApi.getAgentFindings(runId, activeAgent)
      .then(setFindings)
      .catch(console.error)
  ...
  AGENT_TABS.forEach(tab => {
    agentFindingCounts[tab.id] = 0;
  });
  // We'll show count from findings per tab via separate state if needed
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }} className="animate-fade-up">
      ...
      <GlassCard>
        ...
        <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 8, minHeight: 120 }}>
          {findingsLoading ? (
            <div style={{ display: 'flex', justifyContent: 'center', padding: 30 }}><LoadingSpinner /></div>
          ) : findings.length === 0 ? (
