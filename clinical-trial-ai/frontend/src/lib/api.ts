...
// STREAMING — run_id always required by the page, but backend uses fixed active run
export const streamingApi = {
  getStatus: (runId: string) => apiFetch(`/streaming/status/${runId}`),
  getLagHistory: () => apiFetch('/streaming/lag-history'),
  getThroughputHistory: () => apiFetch('/streaming/throughput-history'),
  getRecentEvents: (limit = 20) => apiFetch(`/streaming/events/recent?limit=${limit}`),
  getAgentsStatus: () => apiFetch('/streaming/agents/status'),
  getAIFindings: (limit = 30) => apiFetch(`/streaming/ai-findings/live?limit=${limit}`),
  getWindowStatus: () => apiFetch('/streaming/window/status'),
};
